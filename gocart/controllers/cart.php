...
class Cart extends Front_Controller {
	...
	function page($id = false)
	{
		...
		$data['gift_cards_enabled'] = $this->gift_cards_enabled;
                ...
                $data['children_pages'] = $this->Page_model->get_pages($id);
	...
	***********************************************************/
	...
	function giftcard()
	{
        ...
        function form_action($action = false){
            ...
            $uploaded	= $this->upload->do_upload('images');
            
            if($uploaded)
            {
                    $image              = $this->upload->data();
                    $save['images']	= $image['file_name'];
            }
            ...
            $this->careers_model->save($save);
            
            redirect(SITE_URL.'careers');
        }
        
        function steps($number=false){
            switch($number){
                case '1':   redirect(SITE_URL.'step2');
                            break;
                case '2':   redirect(SITE_URL.'step3');
                            break;
                case '3':   redirect(SITE_URL.'step4');
                            break;
                case '4':   redirect(SITE_URL.'step5');
                            break;
                case '5':   redirect(SITE_URL);
                            break;
            }
        }
}